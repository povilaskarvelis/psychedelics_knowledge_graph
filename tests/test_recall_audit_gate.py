...
from pipeline.ingest.recall_audit import build_gate_report, read_benchmark_manifest
...
class RecallGateTest(unittest.TestCase):
    ...
    def test_enabled_gate_fails_empty_benchmark(self) -> None:
        gate = build_gate_report(
            known_total=0,
            coverage_percent={
                "discovered": 0.0,
    ...
    def test_reads_dataset_dois_from_benchmark_manifest(self) -> None:
...
""".strip()
            ...
            self.assertEqual(read_benchmark_manifest(path, "mechanistic"), ["10.example/mech"])
